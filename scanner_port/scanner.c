...
const char* fingerprint_os(const char* banner) {
    ...
    if (strstr(banner, "Debian")) return "Linux Debian";
    if (strstr(banner, "CentOS")) return "Linux CentOS";
    if (strstr(banner, "Red Hat") || strstr(banner, "RHEL")) return "Linux Red Hat";
    if (strstr(banner, "Alpine")) return "Linux Alpine";
    if (strstr(banner, "FreeBSD")) return "FreeBSD";
    if (strstr(banner, "OpenBSD")) return "OpenBSD";
    if (strstr(banner, "Arch")) return "Arch Linux";

    if (strstr(banner, "Microsoft") || strstr(banner, "Windows")) return "Windows Server";
    if (strstr(banner, "IIS/")) return "Windows Server (IIS)";
    if (strstr(banner, "Win32")) return "Windows (Win32)";
    
    if (strstr(banner, "Darwin")) return "macOS";
    if (strstr(banner, "macOS")) return "macOS";

    if (strstr(banner, "Apache") && strstr(banner, "Debian")) return "Linux Debian (Apache)";
    if (strstr(banner, "Apache") && strstr(banner, "Ubuntu")) return "Linux Ubuntu (Apache)";
